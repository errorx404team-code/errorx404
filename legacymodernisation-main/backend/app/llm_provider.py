...
class LLMProvider:
    ...
    def _fallback_completion(self, prompt: str, json_mode: bool = False) -> str:
        ...
        # Dependency-aware conversion fallback
        if ("cross-file" in prompt_lower or "dependency context" in prompt_lower or "interface contract" in prompt_lower
                or "source file" in prompt_lower and "convert" in prompt_lower):
            # Detect module name from prompt
            import re
            source_match = re.search(r'=== SOURCE FILE ===\s*(\S+)', prompt)
            source_file = source_match.group(1) if source_match else "module.m"
            module_name = re.sub(r'[^a-zA-Z0-9_]', '_', source_file.split('.')[0]).lower()
            # Extract any deps mentioned
            dep_match = re.findall(r'from\s+(\w+)\s+import', prompt)
            import_lines = "\n".join([f"from {d} import {d.title()}Module" for d in dep_match[:3]]) if dep_match else ""
            return f'''# Modernized from: {source_file}
# ErrorX404 dependency-aware conversion (fallback mode)
{import_lines}
from typing import Dict, Any, Optional

class {module_name.title().replace("_", "")}Module:
    """Auto-generated from {source_file} — dependency-aware modernization."""

    def __init__(self):
        # RULE-1: Initialize data stores (MUMPS globals mapped to dict)
        self._data: Dict[str, Any] = {{}}

    def process(self, record_id: str, **kwargs) -> Dict[str, Any]:
        """RULE-1: Main processing logic preserved from {source_file}."""
        if not record_id:
            return {{"status": "ERROR", "message": "record_id is required"}}
        result = {{"record_id": record_id, "status": "PROCESSED"}}
        result.update(kwargs)
        self._data[record_id] = result
        return result

    def get_record(self, record_id: str) -> Optional[Dict[str, Any]]:
        """RULE-2: Retrieve record by ID."""
        return self._data.get(record_id)

    def verify(self, record_id: str) -> bool:
        """RULE-3: Verify record exists and is valid."""
        return record_id in self._data and self._data[record_id].get("status") != "ERROR"
'''
