...
def convert_routine(routine_id: int, req: schemas.ConversionRequest, db: Session = Depends(get_db)):
    ...
    if routine.workspace_id:
        ...
        if relevant_edges or contracts.get(routine.name):
            ...
            generated_code = converter.convert_code_with_context(
    ...
    # Fallback: single-file conversion (existing behavior)
...
"""
                llm_res = llm_provider.generate_completion(
                    prompt,
                    system_instruction="You are a QA automation engineer. Generate realistic test cases based on legacy MUMPS code.",
                    json_mode=True
                )
...
"""
    ...
    # If LLM unavailable, give a transparent fallback (no fake answer)
    if not llm_provider.api_key or llm_provider.api_key == "your_gemini_api_key_here":
        if no_context:
